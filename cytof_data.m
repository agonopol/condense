...
classdef cytof_data
    ...
    methods
        ...
        function obj = cytof_data( filename, name2, well_key)
            ...
            obj.barcoding_channels = obj.find_channel_prefix('mcb');
            ...
            celldeath = obj.find_channel_prefix('celldeath');
            [~, num_death]=size(celldeath);
            
            for j=1:num_death
                
                obj.marker_channels(find(obj.marker_channels == celldeath(j))) = [];
                
            end
            
            filechannel = obj.find_channel_prefix('file');
            [~, num_files]=size(filechannel);
            
            for j=1:num_files
                
                obj.marker_channels(find(obj.marker_channels == filechannel(j))) = [];
                
            end
            
            filechannel = obj.find_channel_prefix('channel');
            [~, num_files]=size(filechannel);
        ...
        function obj = normalize_zscore(obj)
        ...
        function obj = add_data_object(obj, obj2, varargin)
        ...
        function obj = transform_data(obj,c)
            ...
            for i=1:dn_channels
             ...
             [~,bead_channels] = size(obj.bead_channels);
            for i=1:bead_channels
                d=obj.bead_channels(i);
                x = obj.data(:,d);
                %obj.data(:,d) = log(1/c*x + sqrt((1/c*x).^2+1));
                obj.data(:,d) = asinh(x/c);
                %isn't this right?
        ...
        function draw_colored_tsne_plots(obj, channel_names, no_rows, no_cols)
            
            
           channels = zeros(1,length(channel_names)); 
           ...
           for i=1:length(channel_names)
               ...
               obj.draw_tsne(obj.data(:,channels(i)));
        ...
        function plot_2d_channel_dscatter(obj, channel1_name, channel2_name,varargin)
            ...
            optargin = length(varargin);
            if(optargin>0)
                dscatter(channel1_data,channel2_data,varargin{1});
            else
                
                dscatter(channel1_data, channel2_data);
            end
            minx = min(obj.data(:,channel1));
            miny = min(obj.data(:,channel2));
            maxx = max(obj.data(:,channel1));
        ...
        function [obj, sig_edges, sig_mis, sig_edge_matrix] = write_mi_graph(obj, channel_names, mi_matrix, threshold, filename, varargin)
            ...
            for i=1:length(channel_names)
                for j=1:length(channel_names)
                    ...
                    if(ctrl_specified<0)
                        ...
                        s = sprintf('control specified!');
                        [minx1, miny1, maxx1, maxy1] = find_data_cutoffs(obj, channel1_name, channel2_name, 25, 255);
                        [minx2, miny2, maxx2, maxy2] = find_data_cutoffs(ctrl_data, channel1_name, channel2_name, 25, 255);
                        maxy = max(maxy1,maxy2);
                        [mi_matrix(i,j),~] = obj.compute_dremi(channel1_name, channel2_name, noise_threshold, 'maxy', maxy);
                        %[mi_matrix(i,j),~] = obj.compute_twoD_dremi(channel1_name, channel2_name, noise_threshold, 8);
                    end
        ...
        function [mi_matrix,xmaxes,ymaxes ] = channel_pair_mi_compute(obj, xchannel, channel_names, noise_threshold, varargin)
            
            ctrl_specified = -1;
            ctrl_data = [];
            
            mi_matrix = zeros(1, length(channel_names));
            xmaxes = zeros(1, length(channel_names));
            ymaxes = zeros(1, length(channel_names));
            ...
            for i=1:length(varargin)
                ...
                for j=1:length(channel_names)
                    ...
                    if(ctrl_specified<0)
                        ...
                        [minx2, miny2, maxx2, maxy2] = find_data_cutoffs(ctrl_data, channel1_name, channel2_name, 25, 255);
                         maxy = max(maxy1,maxy2);
                         xmaxes(j) = min(maxx1, maxx2);
                         ymaxes(j) = maxy;
                        [mi_matrix(j),~] = obj.compute_dremi(channel1_name, channel2_name, noise_threshold, 'maxy', maxy);
                    end
         ...
         function [obj, selected_cells] = subsample_data(obj, number_sampled,varargin)
             ...
             if(length(obj.time_points)==0)
                 ...
                 obj.time_points = ones(num_points,1);
             end
             
             if(length(varargin)>0)
                 if(strcmp(varargin{1},'sample_info'))
                 
                     sample_labels = varargin{2};
                 
                 end
             else
                
                 sample_labels = obj.time_points;
                 
                
             end
             
             num_points = max(sample_labels)
             samples_per_point = floor(number_sampled/num_points)
             selected_cells=[];
              
              
           for i=1:num_points 
            
                data_length = length(find(sample_labels==i));
                data_indices = find(sample_labels==i);
                if(data_length<samples_per_point)
                   new_sample = data_indices;    
                else    
                    new_sample = randsample(data_length,samples_per_point);
                    new_sample = data_indices(new_sample);
                end
                selected_cells = [selected_cells; new_sample]
                
            
           end
            
           obj.data = obj.data(selected_cells,:);
           obj.time_points = obj.time_points(selected_cells,:);
        ...
        function [noise_DREMI] = compute_noise_dremi(obj, channel1_name, channel2_name)
            ...
            [noise_DREMI, samples_x, samples_y] = compute_dremi_densitymatrix(shifted_normalized_density,xaxis, yaxis, .6);
        ...
        function [DREMI_values, Traj_time] = compute_windowed_DREMI_interpolate(obj, window_channel, channel1_name, channel2_name, steps, prob_threshold, num_partitions)
        ...
        function [DREMI, DREMIXZ, DREMIYZ ] = compute_threeD_dremi(obj, channel1_name, channel2_name,channel3_name, prob_threshold, num_partitions,varargin)
            ...
            [estimated_points, meshX, meshY, meshZ, ~, ~, ~, cellhist, estimated_points_joint] =  obj.threeD_edge_visualize( channel1_name, channel2_name, channel3_name,varargin);
            
            estimated_points_joint_flat = zeros(size(estimated_points,1),size(estimated_points,2));
        ...
        function [estimated_points, meshX, meshY, conditional_mean, meshX1d, cellhist, edgesx] = twoD_edge_visualize(obj, channel1_name, channel2_name,varargin)
            ...
            [ estimated_points, meshX, meshY, conditional_mean, meshX1d] = conditional_kde(edge_data,128,varargin);
        ...
        function [estimated_points, meshX, meshY, meshZ, conditional_mean, meshX2d, meshY2d, cellhist, edgesx, edgesy] = threeD_edge_visualize(obj, channel1_name, channel2_name, channel3_name, varargin)
            ...
            edge_data = obj.data(:,[channel1 channel2 channel3]);
            %[ estimated_points, meshX, meshY, meshZ, conditional_mean, meshX2d, meshY2d] = conditional_kde_3d(edge_data,80,varargin);
            ...
            figure
            colormap(jet);
            view_2dslice(estimated_points, meshX, meshY, meshZ, {channel1_name, channel2_name, channel3_name},1,  .2);
            ...
            figure
            colormap(jet);
            view_2dslice(estimated_points, meshX, meshY, meshZ, {channel1_name, channel2_name, channel3_name},1, .8);
            zlim([0 zmax]);
        end
...
%                 
        ...
        function [difference_vector] = cluster_L1_difference(obj, cluster_vector1, cluster_num1, cdata2, cluster_vector2,cluster_num2, channels)
            
            cluster1_indices = find(cluster_vector1==cluster_num1);
            cluster2_indices = find(cluster_vector2==cluster_num2);
            
            difference_vector = zeros(1, length(channels));
            
            for j=1:length(channels)
                
                cluster1_data = obj.get_channel_data(channels{j});
                cluster1_data = cluster1_data(cluster1_indices);
                
                cluster2_data = cdata2.get_channel_data(channels{j});
                cluster2_data = cluster2_data(cluster2_indices);
                
                [f, xi] = ksdensity([cluster1_data; cluster2_data],'function','cdf');
                %automatically finds the points so then I will use them.
                
                f1 = ksdensity(cluster1_data, xi,'function','cdf');
                ctrl = ksdensity(cluster2_data, xi,'function','cdf');

               difference_vector(j)  = norm(f1-ctrl,1);
                
           end
      end
      
      function [difference_matrix] = all_cluster_L1_difference(obj, cluster_vector, sample1, sample2, channels)
          
         difference_matrix = [];
         for i=1:max(cluster_vector)
             difference_vector = obj.cluster_L1_difference_alt(cluster_vector, i, sample1, i, sample2, channels);
             difference_matrix = [difference_matrix; difference_vector];
         end
         
      end
      
%       function [difference_vector] = cluster_L1_difference_alt(obj, cluster_vector, cluster_num1, sample1 , cluster_num2, sample2, channels)
%           
%            cluster1_indices = intersect(find(cluster_vector==cluster_num1), find(obj.time_points==sample1));
%            cluster2_indices = intersect(find(cluster_vector==cluster_num2), find(obj.time_points==sample2));
%            cdata1 = obj;
%            cdata2 = obj;
%            cdata1.data=obj.data(cluster1_indices,:);
%            cdata2.data=obj.data(cluster2_indices,:);
%            
%           
%            
%            difference_vector = zeros(1, length(channels));
...
%            for j=1:length(channels)
% 
%                 cluster1_data = cdata1.get_channel_data(channels{j});
%                
%                 
%             end
%         end
        ...
        function [difference_vector] = cluster_L1_difference_alt(obj, cluster_vector, cluster_num1, sample1 , cluster_num2, sample2, channels)
            ...
            for j=1:length(channels)
               ...
               if(mean(cluster2_data)<mean(cluster1_data))
                   
                   difference_vector(j) = difference_vector(j)*(-1);
               end
        ...
        function [obj, gated_indices] = manual_gate_events_box(obj, channel_name1, channel_name2, left, bottom, width, height)
       ...
       function [obj] = manual_gate_events_box_out(obj, channel_name1, channel_name2, left, bottom, width, height)
            ...
            obj.data(gated_indices,:)=[];
            
            if(length(obj.time_points)>0)
             
                obj.time_points(gated_indices)=[];
            end    
        end
        ...
        function [smooth_data, smooth_index ] = plot_events_vs_time(obj, channel_name, window_size, varargin)
            ...
            %wanderlust_channel = obj.name_channel_map('time');
            wanderlust_channel = 1;
            if(optargin > 0)
        ...
        function [Ls_powered] = walk_laplacian(obj, channels, time, varargin)
            [~,Ls] = compute_laplacian(obj, 'channels', channels, varargin{:})
            Ls_powered = Ls;
            for i=2:time
                Ls = Ls_powered * Ls;
            end
        ...
        function [f] = compute_diffusion_density(obj, channel_names, time, k, varargin)
           
            Ls_powered = obj.walk_laplacian(channel_names,time,varargin{:});
            
            [idx,d] = knnsearch(Ls_powered, Ls_powered, 'distance', 'euclidean', 'K', k+1,'NSMethod','kdtree');
            radius = d(:,k+1);
            %f = (k)./(num_cells*pi*(radius.^2));
            f = (k)./(num_cells*pi*(radius.^5));
            
        end
      
        function [f] = poisson_subsample_channels(obj,channel_names, lambda)
            
            
        end
        
        function [f] = compute_knn_density(obj, channel_names,k)
        ...
        function [f] = compute_multi_density(obj, channel_names)
            ...
            M = obj.data(:,channels);
            q = kde(transpose(M),'rot');
            f = evaluate(q, transpose(M));
        ...
        function obj = zscore_normalise(obj, col_ind)
            disp 'zscore normalise per gene per sample (time_point)'
            for I=unique(obj.time_points)'
                I
                ind = obj.time_points == I;
                data_norm = zscore(obj.data(ind,col_ind));
                obj.data(ind,col_ind) = data_norm;
            end
        end

...
%              obj.data = data_norm';           
       ...
       function [obj] = compute_outliers(obj,k, indegree_t)
        ...
        function [dremi] = knn_dremi(obj, channel_x, channel_y, threshold, varargin)
            ...
            for i=1:length(varargin)-1
                ...
                if(strcmp(varargin{i},'diffusion'))
                    use_diffusion = varargin{i+1};
                end
            ...
            if(use_diffusion)
                ...
                [fc,fj,fp] = obj.knn_conditional_density({channel_x,channel_y}, {channel_x}, 'diffusion', true,'rescale', true, 'k_rescale', 20, 'k_knn', 50, 'draw_plot', true);
                 
            else
                
                [fc,fj,fp] = obj.knn_conditional_density({channel_x,channel_y}, {channel_x}, 'diffusion', false,'rescale', true, 'k_rescale', 20, 'k_knn', 50, 'draw_plot', true);
...
%                 xdensity = euclidean_diffusion_density(obj, 'channels', {channel_x}, varargin{:});
%                 ydensity = euclidean_diffusion_density(obj, 'channels', {channel_y}, varargin{:});
%                 joint_density = euclidean_diffusion_density(obj, 'channels', {channel_x, channel_y}, varargin{:});
                
                xdensity = obj.centrality_diffusion('channels', {channel_x},  varargin{:});
                ydensity = obj.centrality_diffusion('channels', {channel_y},  varargin{:});
                joint_density = obj.centrality_diffusion(obj, 'channels', {channel_x, channel_y}, varargin{:});
            else
                
                xdensity = euclidean_density(obj, 'channels', {channel_x}, varargin{:});
                ydensity = euclidean_density(obj, 'channels', {channel_y}, varargin{:});
                joint_density = euclidean_density(obj, 'channels', {channel_x, channel_y}, varargin{:});

            end
            
             %implement the threshold.
             xdensity(fc<threshold)=[];
             ydensity(fc<threshold)=[];
             joint_density(fc<threshold)=[];
             fc(fc<threshold) = [];
              
             [entropy_x] = compute_plug_in_entropy_estimate(obj, xdensity, k_density, 'weights', fc);
             %maybe I should get rid of this value, or would it be the 
             [entropy_y] = compute_plug_in_entropy_estimate(obj, ydensity, k_density, 'weights', fc);
             [entropy_joint] = compute_plug_in_entropy_estimate(obj, joint_density, k_density, 'weights', fc);
             dremi = entropy_x + entropy_y - entropy_joint;
              
            
           
        end       
                
        function [mutual_information, joint_density] = knn_mutual_information(obj, channel_x, channel_y, varargin)
                    
            use_diffusion = false;
           
            k_density = ceil(size(obj.data,1)^(1/(1+2)));
            k_density1 = ceil(size(obj.data,1)^(1/(1+1)));
            k_density2 = ceil(size(obj.data,1)^(1/(1+2)));
            %k_density = 10;
            renyi = false;
            throw_zeros = false;
            ...
            for i=1:length(varargin)-1
               ...
               if(strcmp(varargin{i},'renyi'))
                  
                   renyi = varargin{i+1};
               end
               if(strcmp(varargin{i}, 'throw_zeros'))
                   
                   throw_zeros = varargin{i+1};    
             ...
             if(renyi)
                 ...
                 %renyi mi 
                  mutual_information = sum((joint_density./(xdensity.*ydensity)).^-0.5)*1/length(xdensity);
             else
                 
               
                %shannon mi    
                [entropy_x] = compute_plug_in_entropy_estimate(obj, xdensity, k_density1);
                [entropy_y] = compute_plug_in_entropy_estimate(obj, ydensity, k_density1);
                [entropy_joint] = compute_plug_in_entropy_estimate(obj, joint_density, k_density2);
